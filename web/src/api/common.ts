...
export function fileUpload(fd: FormData, params: anyObj = {}, forceLocal = false, config: AxiosRequestConfig = {}): ApiPromise {
    ...
    if (!file.name || typeof file.size == 'undefined') {
        errorMsg = i18n.global.t('util.The data of the uploaded file is incomplete!')
    } else if (!checkFileMimetype(file.name, file.type)) {
        errorMsg = i18n.global.t('util.The type of uploaded file is not allowed!')
    } else if (file.size > siteConfig.upload.maxsize) {
        errorMsg = i18n.global.t('util.The size of the uploaded file exceeds the allowed range!')
    }
    if (errorMsg) {
...
export function refreshToken() {
    ...
    return createAxios({
        ...
        method: 'POST',
        data: {
            refreshToken: adminInfo.getToken(),
        },
    })
}
