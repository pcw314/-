...
export const isAdminApp = (path = '') => {
    const regex = new RegExp(`^${baseRoutePath}`)
    if (path) {
...
export const __ = (key: string, named?: Record<string, unknown>, options?: TranslateOptions<string>) => {
    ...
    const path = getCurrentRoutePath()
    if (isAdminApp()) {
        langPath = path.slice(path.indexOf(baseRoutePath) + baseRoutePath.length)
...
export const timeFormat = (dateTime: string | number | null = null, fmt = 'yyyy-mm-dd hh:MM:ss') => {
    ...
    if (dateTime.toString().length === 9 || dateTime.toString().length === 10) {
...
export const getGreet = () => {
    ...
    if (hour < 5) {
        greet = i18n.global.t('util.Late at night, pay attention to your body!')
    } else if (hour < 9) {
        greet = i18n.global.t('util.good morning!') + i18n.global.t('util.welcome back')
    } else if (hour < 12) {
        greet = i18n.global.t('util.Good morning!') + i18n.global.t('util.welcome back')
    } else if (hour < 14) {
        greet = i18n.global.t('util.Good noon!') + i18n.global.t('util.welcome back')
    } else if (hour < 18) {
        greet = i18n.global.t('util.good afternoon') + i18n.global.t('util.welcome back')
    } else if (hour < 24) {
        greet = i18n.global.t('util.Good evening') + i18n.global.t('util.welcome back')
    } else {
        greet = i18n.global.t('util.Hello!') + i18n.global.t('util.welcome back')
    }
    return greet
}
