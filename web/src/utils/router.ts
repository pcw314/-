...
import baseRoute from '/@/router/static/base'
...
export const routePush = async (to: RouteLocationRaw) => {
    try {
        ...
        if (isNavigationFailure(failure, NavigationFailureType.aborted)) {
            ElNotification({
                message: i18n.global.t('util.Navigation failed, navigation guard intercepted!'),
                type: 'error',
            })
        } else if (isNavigationFailure(failure, NavigationFailureType.duplicated)) {
            ElNotification({
                message: i18n.global.t('util.Navigation failed, it is at the navigation target position!'),
                type: 'warning',
            })
        }
    } catch (error) {
        ElNotification({
            message: i18n.global.t('util.Navigation failed, invalid route!'),
            type: 'error',
        })
        console.error(error)
    }
...
export const onClickMenu = (menu: RouteRecordRaw) => {
    switch (menu.meta?.menu_type) {
        ...
        default:
            ElNotification({
                message: i18n.global.t('util.Navigation failed, the menu type is unrecognized!'),
                type: 'error',
            })
            break
    }
...
export const handleAdminRoute = (routes: any) => {
    ...
    addRouteAll(viewsComponent, routes, baseRoute.name as string)
    const menuBaseRoute = baseRoute.path as string

    // 更新stores中的路由菜单数据
    const navTabs = useNavTabs()
    navTabs.setTabsViewRoutes(handleMenuRule(routes, menuBaseRoute))
    navTabs.fillAuthNode(handleAuthNode(routes, menuBaseRoute))
}
...
export const addRouteItem = (viewsComponent: Record<string, any>, route: any, parentName: string, analyticRelation: boolean) => {
    ...
    if (route.menu_type == 'iframe') {
        path = baseRoute.path + '/iframe/' + encodeURIComponent(route.url)
