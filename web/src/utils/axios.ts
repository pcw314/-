...
// import { isAdminApp } from '/@/util/common'
...
function createAxios<Data = any, T = ApiPromise<Data>>(axiosConfig: AxiosRequestConfig, options: Options = {}, loading: LoadingOptions = {}): T {
    ...
    // 自定义后台入口
    // if (baseRoute.path != '/admin' && isAdminApp() && /^\/admin\//.test(axiosConfig.url!)) {
    //     axiosConfig.url = axiosConfig.url!.replace(/^\/admin\//, baseRoute.path + '.php/')
    // }
    ...
    Axios.interceptors.request.use(
        (config) => {
            ...
            if (config.params) {
                ...
                if (search) {
                    config.params.search = JSON.stringify(search)
                }
            }
            return config
        },
